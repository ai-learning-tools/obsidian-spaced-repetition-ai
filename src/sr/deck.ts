...
import { IQuestionPostponementList } from "@/sr/questionPostponementList";
import { TopicPath, TopicPathList } from "@/sr/topicPath";
...
export class Deck {
    ...
    //
    public getQuestionCardCount(qa: QA): number {
        let result: number = 0;
        result += this.getQuestionCardCountForCardListType(qa, this.newFlashcards);
        result += this.getQuestionCardCountForCardListType(qa, this.dueFlashcards);
        return result;
    }

    private getQuestionCardCountForCardListType(qa: QA, cards: Card[]): number {
        let result: number = 0;
        for (let i = 0; i < cards.length; i++) {
            if (Object.is(qa, cards[i].qa)) result++;
        }
        return result;
    }
    ...
    //
    deleteQuestionFromAllDecks(qa: QA, exceptionIfMissing: boolean): void {
        for (const card of qa.cards) {
            this.deleteCardFromAllDecks(card, exceptionIfMissing);
        }
    }

    deleteQuestion(qa: QA, exceptionIfMissing: boolean): void {
        for (const card of qa.cards) {
    ...
    deleteCardFromAllDecks(card: Card, exceptionIfMissing: boolean): void {
        for (const topicPath of card.qa.topicPathList.list) {
    ...
    copyWithCardFilter(predicate: (value: Card) => boolean, parent: Deck | null = null): Deck {
...
export class DeckTreeFilter {
    static filterForReviewableCards(reviewableDeckTree: Deck): Deck {
        return reviewableDeckTree.copyWithCardFilter((card) => !card.qa.hasEditLaterTag);
    }
    ...
    static filterForRemainingCards(
        ...
        return deckTree.copyWithCardFilter(
            (card) =>
                ...
                !questionPostponementList.includes(card.qa),
        );
    }
}
